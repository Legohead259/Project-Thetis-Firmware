...
void setup() {
    ...
    pinMode(LOG_EN_PIN, INPUT_PULLUP);
...
void testIMU() {
    ...
    while (millis() < startTime + TEST_TIME*6) { // For TEST_TIME, read off IMU data and update LEDs at SAMPLE_RATE
        ...
        digitalWrite(ACT_LED_PIN, system==3);
        digitalWrite(GCAL_LED_PIN, gyro);
        digitalWrite(ACAL_LED_PIN, accel);
        digitalWrite(MCAL_LED_PIN, mag);
